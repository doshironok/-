...
namespace GraphLibraryTests
{
    ...
    public class GraphTests
    {
        ...
        [Test]
        public void BFS_From_0_ShouldVisit_0_then_1_then_2()
        {
            var graph = new Graph(); // конструктор без параметров
            ...
            graph.DirectLoadFromIncidenceMatrix(matrix); // ← прямая загрузка

            var result = graph.BFS(0);

            ClassicAssert.AreEqual(new List<int> { 0, 1, 2 }, result);
        }

        [Test]
        public void BuildAdjacencyList_InvalidEdge_OnlyFrom_IgnoresEdge()
        {
            var reader = new FakeDisconnectedGraphReader();
            var writer = new FakeAdjacencyWriter();
            var graph = new Graph(reader, writer);
            // Ребро: только начало, нет конца
            var matrix = new List<List<int>>
            {
                new List<int> { -1 }, // узел 0
                new List<int> { 0 },  // узел 1 — не участвует
                new List<int> { 0 }   // узел 2 — не участвует
            };

            graph.LoadFromIncidenceMatrix(matrix);
            var result = graph.BFS(0);

            // Ожидаем: только [0], так как ребро некорректно
            Assert.That(result, Is.EqualTo(new[] { 0 }));
        }

        [Test]
        public void SaveToAdjacencyMatrix_ShouldHaveOnesAtEdges()
        {
            var reader = new FakeIncidenceReader();
            var spy = new SpyAdjacencyWriter();
            var graph = new Graph(reader, spy);
            ...
            graph.LoadFromIncidenceMatrix("");
            graph.SaveToAdjacencyMatrix("out");
            // Убивает мутант: matrix[i][j] = 1 yf matrix[i][j] = 0
            var expected = new List<List<int>>
    {
                new List<int> { 0, 1, 0 },
                new List<int> { 1, 0, 1 }, // явная проверка
                new List<int> { 0, 1, 0 }
    };
            Assert.That(spy.SavedMatrix, Is.EqualTo(expected));
        }
